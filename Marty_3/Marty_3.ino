...
#define TRIG_PIN 12 // define the pins of your sensor
#define ECHO_PIN 13
...
#define TURN_DIST COLL_DIST+10 // sets distance at which robot veers away from object (not reverse) to 20cm 
...
void setup() {
  ...
  sonarServo.write(90); // make sure servo is still facing forward
...
void checkPath() {
  ...
  int curRight = 0;
  int curDist = 0;
  sonarServo.write(144); // set servo to face left 54-degrees from forward
...
void checkBackward() {
  if (motorSet == "BACKWARD") {
    motor1->run(BACKWARD);  // make sure motors are going backward
    motor2->run(BACKWARD);
  }
}
//-------------------------------------------------------------------------------------------------------------------------------------
